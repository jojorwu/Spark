...
impl RenderPass for VolumetricPass {
    ...
    fn destroy(&mut self, renderer: &mut Renderer) {
        ...
        unsafe {
            for a in self.output_images.drain(..) {
                a.destroy(device, &renderer.device.allocator);
            }
...
impl VolumetricPass {
    ...
    pub fn update_descriptor_sets(&self, renderer: &Renderer) {
        for i in 0..crate::MAX_FRAMES_IN_FLIGHT {
            let out_info = [vk::DescriptorImageInfo::default()
                .image_layout(vk::ImageLayout::GENERAL)
                .image_view(self.output_images[i].view)];
            let depth_info = [vk::DescriptorImageInfo::default()
                .image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL)
                .image_view(
                    renderer
    ...
    pub fn record_commands_impl(
        ...
        unsafe {
            ...
            device.cmd_bind_descriptor_sets(
                ...
                self.layout,
                0,
                &[global_ds, self.descriptor_sets[current_frame]],
                &[],
            );
            ...
            device.cmd_dispatch(
                cb,
                (extent.width / 2).div_ceil(8),
                (extent.height / 2).div_ceil(8),
                1,
            );
