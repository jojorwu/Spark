...
impl RenderPass for TAAPass {
    ...
    fn prepare(&self, renderer: &Renderer, current_frame: usize) {
        ...
        let current_info = [vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(renderer.gbuffer_hdr[current_frame].view).sampler(sampler)];
        ...
        let velocity_info = [vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(renderer.gbuffer_velocity[current_frame].view).sampler(sampler)];
        let depth_info = [vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(renderer.gbuffer_depth[current_frame].view).sampler(sampler)];
