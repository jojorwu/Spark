...
impl RenderPass for SSRPass {
    ...
    fn prepare(&self, renderer: &Renderer, current_frame: usize) {
        ...
        let img_infos = [
            vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(renderer.gbuffer_albedo[current_frame].view).sampler(sampler),
            vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(renderer.gbuffer_normal[current_frame].view).sampler(sampler),
            vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(renderer.gbuffer_pbr[current_frame].view).sampler(sampler),
            vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(renderer.gbuffer_depth[current_frame].view).sampler(sampler),
