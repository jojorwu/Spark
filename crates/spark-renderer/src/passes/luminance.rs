...
impl RenderPass for LuminancePass {
    fn name(&self) -> &str { "LuminancePass" }
    fn inputs(&self) -> Vec<&'static str> { vec!["HDRColor"] }
    fn outputs(&self) -> Vec<&'static str> { vec!["Luminance"] }
    ...
    fn record_commands(&self, ctx: &RenderContext) {
        ...
        unsafe {
            ...
            let pc_bytes = std::slice::from_raw_parts(&pc as *const _ as *const u8, std::mem::size_of::<LuminancePC>());
    ...
    fn get_resource_buffer(&self, name: &str) -> Option<Buffer> {
        if name == "Luminance" { Some(self.luminance_buffer.clone()) } else { None }
    }
...
impl LuminancePass {
    pub fn new(renderer: &Renderer, shader_spirv: &[u32]) -> Result<Self, crate::error::RendererError> {
        ...
        let layout = unsafe { device.create_pipeline_layout(&vk::PipelineLayoutCreateInfo::default().set_layouts(&[ds_layout]).push_constant_ranges(&[vk::PushConstantRange { stage_flags: vk::ShaderStageFlags::COMPUTE, offset: 0, size: 32 }]), None)? };
        ...
        Ok(Self { pipeline, layout, descriptor_set_layout: ds_layout, descriptor_sets, luminance_buffer })
    }
}
