...
impl RenderPass for DoFPass {
    fn name(&self) -> &str { "DoFPass" }
    fn inputs(&self) -> Vec<&'static str> { vec!["HDRColor", "GBuffer"] }
    fn outputs(&self) -> Vec<&'static str> { vec!["DoF"] }
    ...
    fn prepare(&self, renderer: &Renderer, current_frame: usize) {
        ...
        let hdr_info = [vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(hdr_view).sampler(sampler)];
        let depth_info = [vk::DescriptorImageInfo::default().image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL).image_view(depth_view).sampler(sampler)];
        ...
        let writes = [
            vk::WriteDescriptorSet::default().dst_set(self.descriptor_sets[current_frame]).dst_binding(0).descriptor_type(vk::DescriptorType::COMBINED_IMAGE_SAMPLER).image_info(&hdr_info),
            ...
            vk::WriteDescriptorSet::default().dst_set(self.descriptor_sets[current_frame]).dst_binding(2).descriptor_type(vk::DescriptorType::STORAGE_IMAGE).image_info(&out_info),
        ];
    ...
    fn record_commands(&self, ctx: &RenderContext) {
        ...
        unsafe {
            renderer.device.device.cmd_bind_pipeline(ctx.command_buffer, vk::PipelineBindPoint::COMPUTE, self.pipeline);
            renderer.device.device.cmd_bind_descriptor_sets(ctx.command_buffer, vk::PipelineBindPoint::COMPUTE, self.layout, 0, &[self.descriptor_sets[ctx.current_frame]], &[]);
            ...
            let pc = DoFPC {
                ...
                enabled: if renderer.settings.enable_dof { 1.0 } else { 0.0 },
            };
            let pc_bytes = std::slice::from_raw_parts(&pc as *const _ as *const u8, std::mem::size_of::<DoFPC>());
            renderer.device.device.cmd_push_constants(ctx.command_buffer, self.layout, vk::ShaderStageFlags::COMPUTE, 0, pc_bytes);

            renderer.device.device.cmd_dispatch(ctx.command_buffer, (extent.width + 15) / 16, (extent.height + 15) / 16, 1);
        }
    }
...
impl DoFPass {
    pub fn new(renderer: &Renderer, shader_spirv: &[u32]) -> Result<Self, crate::error::RendererError> {
        ...
        let layout = unsafe { device.create_pipeline_layout(&vk::PipelineLayoutCreateInfo::default().set_layouts(&[ds_layout]).push_constant_ranges(&[vk::PushConstantRange { stage_flags: vk::ShaderStageFlags::COMPUTE, offset: 0, size: 16 }]), None)? };
        ...
        for _ in 0..MAX_FRAMES_IN_FLIGHT {
            output_images.push(Attachment::create_image_resource(&renderer.device, extent.width, extent.height, vk::Format::R16G16B16A16_SFLOAT, vk::ImageUsageFlags::STORAGE | vk::ImageUsageFlags::SAMPLED, vk::SampleCountFlags::TYPE_1)?);
        }
        ...
        let pipeline = unsafe { device.create_compute_pipelines(vk::PipelineCache::null(), &[vk::ComputePipelineCreateInfo::default().stage(stage).layout(layout)], None).map_err(|e| e.1)?[0] };
