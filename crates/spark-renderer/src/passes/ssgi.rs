...
impl SSGIPass {
    pub fn new(renderer: &Renderer, _shader_spirv: &[u32]) -> Result<Self, crate::error::RendererError> {
        ...
        let output_images = (0..MAX_FRAMES_IN_FLIGHT).map(|_| {
            Attachment::create_image_resource(&renderer.device, extent.width, extent.height, vk::Format::R16G16B16A16_SFLOAT, vk::ImageUsageFlags::COLOR_ATTACHMENT | vk::ImageUsageFlags::SAMPLED, vk::SampleCountFlags::TYPE_1).unwrap()
        }).collect::<Vec<_>>();
