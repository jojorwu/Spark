...
impl Renderer {
    pub fn setup_default_passes(&mut self, shaders: PassShaders) -> Result<(), RendererError> {
        ...
        let mut lighting_pass = crate::passes::lighting::LightingPass::new(&self.device.device, pool, layout)
            .map_err(|_| RendererError::NoSuitableDevice)?;
        ...
        let grid_pass = crate::passes::grid::GridPass::new(
            &self.device.device, cache, &shaders.grid_vert, &shaders.grid_frag, layout, vk::Format::R16G16B16A16_SFLOAT
        )?;
